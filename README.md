...
- `/inventory` 提供仓库商品管理，作为库管库存资产台账记录设计留样、客户退货、常备采购和其他库存，并支持总数 / 可用数 / 已占用数展示、库管快捷视图、低库存预警、库存详情来源追溯、关联商品行占用 / 出库追溯、质检处置、库存盘点、关联商品行的前端 mock 入库、占用、释放、出库、报废、库位调整和流转记录筛选
