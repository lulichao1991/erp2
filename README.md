...
legacy `/orders` 清理已经收口，当前进入以 `OrderLine` 为核心的业务工作流 v2 固化阶段。项目仍以前端 UI、页面结构、组件复用和 mock 数据联调为主；真实后端接入前先做字段契约评审，不直接写 API 适配层。
...
当前阶段进入 **OrderLine workflow v2 hardening**：

- v1 foundation 已完成，v2 重点是跨页面一致性、流程固化、真实接口前字段契约和端到端样例验收
...
- 每条销售草稿可搜索、筛选、预览并引用款式，也可覆盖本次购买的实际材质、尺寸、工艺、印记和特殊需求
- 客服确认前展示资料完整度，确认后按设计 / 建模需求分流到后续 `lineStatus`
- `/production-follow-up` 提供跟单生产跟进视图，基于 `OrderLine` 展示待审核、待下发、生产中、待回传、完工待审核和异常 / 逾期销售
- `/design-modeling` 提供设计 / 建模工作台，基于 `OrderLine` 展示待设计、设计中、待建模、建模中、待修改和已完成任务
- `/factory` 提供工厂协同中心，只展示分配给当前 mock 工厂的销售生产资料、回传表单、异常 / 恢复动作和回传附件；提交回传前必须补齐重量、材质、基础工费、成品图和结算单
- `/finance` 提供财务中心，以 OrderLine 货号台账展示应收、收款、退款、净收、待收、补款 / 退款复核、退款原因补录与异常解除、商品行成本卡、旧金抵扣入库关联、FIFO 库存领用成本、工厂回传数据、结算确认、异常标记和锁定；确认结算前校验尾款、补 / 退款复核、退款原因、财务风险和工厂结算金额；财务确认或锁定后对应货号的收退款、结算金额、备注和异常处理进入只读；Purchase 只做聚合摘要
- `/inventory` 提供仓库商品管理，作为库管库存资产台账记录设计留样、客户退货、旧金抵扣入库、常备采购和其他库存，并支持 FIFO 批次、总数 / 可用数 / 已占用数展示、库位汇总、库管快捷视图、可领用 / 待出库 / 待盘点视图、低库存预警、库存详情来源追溯、关联销售占用 / 出库追溯、质检处置、库存盘点、关联销售的前端 mock 入库、占用、释放、出库、报废、库位调整和流转记录筛选
- `/management` 提供管理看板，汇总 Purchase、OrderLine、生产风险、财务概览、角色负载和工厂表现
- 前端 mock role capability 已覆盖客服、跟单、设计、建模、工厂、库管、财务、管理、管理员的导航可见性和关键动作边界
- 销售资料完整性、生产逾期、工厂回传异常、财务异常和角色待办徽标统一由 `orderLineRiskSelectors` 计算
- 新 PRD 名词按 current model 映射：`order -> Purchase`、`item/work_order -> OrderLine.productionTaskNo + OrderLine`、`style_template -> Product`、`payment -> FinancePaymentRecord`
- 接真实接口前先按 `docs/frontend/interface-readiness.md` 评审字段契约和验收边界，不在当前 mock 阶段直接写 API。
...
- 销售中心一行代表一件商品
- 销售详情页承接单件销售的状态推进、物流、售后、设计建模、生产和工厂回传，不恢复 legacy `/orders`
- 购买记录用于保存一次购买中的公共信息
- 购买记录详情页是归组页，展示本次购买下的所有销售
- 任务中心只作为协作提醒和跟进入口，不替代 `OrderLine.lineStatus` 主流程
- 生产跟进页按销售筛选生产推进状态，不回退到旧订单单件逻辑；工厂回传后先进入 `factory_returned` 完工待审核，跟单审核通过后才进入 `pending_finance_confirmation`
- 生产计划页只从当前 `tasks + purchases + orderLines + products` 生成视图，不读取旧订单模型；全定制销售没有来源 `Product` 时使用 `OrderLine` 自身生产资料生成计划行
- 设计建模页只展示商品执行需求、设计 / 建模状态和文件记录，不展示客户隐私或财务金额
- 工厂协同页只展示分配给当前工厂的生产资料、设计 / 建模 / 出蜡文件、回传字段、异常 / 恢复动作和回传附件，不展示客户联系方式、销售价格、定金、尾款、利润或财务备注
...
- 跟单：处理生产跟进、工厂计划、下发动作和工厂回传后的完工审核
- 设计 / 建模：处理设计建模工作台和文件记录
- 工厂：只看工厂任务和生产回传，不看客户联系方式、销售价格、利润或财务备注
- 库管：处理库存资产和库存流转，不推进销售执行状态
- 财务：处理收款、补款、退款、工厂结算、成本确认和财务异常；确认或锁定后的货号只读
...
- `/products`：款式管理
- `/products/new`：新建款式
- `/products/:productId`：款式详情
- `/products/:productId/edit`：编辑款式
...
### InventoryItem
仓库商品 / 库存资产，用于记录设计部门留样、客户退货入库、旧金抵扣入库、常备采购、寄售和其他库存。

库存记录可以关联 `Product / Purchase / OrderLine / Customer` 做来源追溯；旧金抵扣入库还可以通过 `sourcePaymentRecordId` 追溯财务流水，并用 `valuationAmount` 记录抵扣估值。每次入库形成 FIFO 批次；关联销售的出库按批次先进先出计入该 `OrderLine` 成本卡，报废只扣库存批次不进入销售成本。库存可由库管做质检处置、盘点或记录关联销售的占用 / 出库；库存台账会明确展示总数、可用数和已占用数，库存工作台徽标由统一 selector 计算。库存变更不替代款式模板，也不驱动销售执行状态。
...
- 核心 workflow 状态字段已经冻结为受控 union，不再接受任意 `string`；未知后端状态必须先在接口边界映射、拒绝或更新契约
...
- `productionInfo` 只放展示型工厂回传文本与回传子状态，结构化重量、成本、附件和结算数据放在 `productionData`
- `FinancePaymentRecord` 是商品行收付款流水权威来源；`Purchase.finance` 只做整笔购买聚合摘要
- 客服确认动作按 `requiresDesign / requiresModeling` 分流，不把单件执行状态塞回 `Purchase`

### 5. 旧模块不作为当前兼容层
- 旧 `/orders` 和旧命名只通过 git 历史追溯；当前 runtime 不保留兼容入口

---

## 当前阶段不做

当前 mock v2 阶段暂不处理以下能力：
...
被删除或归档的历史规划只通过 git 历史追溯；当前开发不再以它们作为约束。
