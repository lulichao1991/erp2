...
- `/inventory` 提供仓库商品管理，作为库管库存资产台账记录设计留样、客户退货、常备采购和其他库存，并支持库管快捷视图、库存详情来源追溯、质检处置、前端 mock 入库、占用、释放、出库、报废和库位调整记录
...
库存记录可以关联 `Product / Purchase / OrderLine / Customer` 做来源追溯，并可由库管做质检处置；库存变更不替代产品模板，也不驱动商品行执行状态。
