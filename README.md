...
- `/inventory` 提供仓库商品管理，作为库管库存资产台账记录设计留样、客户退货、常备采购和其他库存，并支持库管快捷视图、低库存预警、库存详情来源追溯、质检处置、关联商品行的前端 mock 入库、占用、释放、出库、报废、库位调整和流转记录筛选
...
库存记录可以关联 `Product / Purchase / OrderLine / Customer` 做来源追溯，并可由库管做质检处置或记录关联商品行的占用 / 出库；库存变更不替代产品模板，也不驱动商品行执行状态。
