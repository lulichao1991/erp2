...
const renderRouteWithTestNavigation = (entry: string) =>
  render(
    <MemoryRouter initialEntries={[entry]}>
      <AppDataProvider>
        <AppRouter />
...
describe('router smoke', () => {
  it('renders product detail route', () => {
    ...
    expect(screen.getByRole('heading', { name: '款式详情' })).toBeInTheDocument()
    expect(screen.getAllByText('山形素圈戒指').length).toBeGreaterThan(0)
    expect(screen.getAllByText('2/2 启用').length).toBeGreaterThan(0)
    expect(screen.getAllByText('¥ 1,280 - ¥ 1,450').length).toBeGreaterThan(0)
    expect(screen.getAllByText('4/4 启用').length).toBeGreaterThan(0)
    expect(screen.getByText('材质 / 工艺 / 特殊需求')).toBeInTheDocument()
    expect(screen.getByText('参考文件')).toBeInTheDocument()
    expect(screen.getAllByText('4 份').length).toBeGreaterThan(0)
  })

  it('renders full product edit form and keeps design version read-only', async () => {
    const user = userEvent.setup()
    renderRoute('/products/p-ring-001/edit')

    expect(screen.getByRole('heading', { name: '编辑款式' })).toBeInTheDocument()
    expect(screen.getByLabelText('当前版本')).toHaveTextContent('v3')
    expect(screen.getByRole('heading', { name: '定制规则' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '生产参考' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '图片与文件' })).toBeInTheDocument()
    expect(screen.getByLabelText('详情图片上传')).toBeInTheDocument()
    expect(screen.getByLabelText('建模文件上传')).toBeInTheDocument()
  ...
  it('creates a new design version under the same product', async () => {
    const user = userEvent.setup()
    renderRoute('/products/p-ring-001/edit')

    expect(screen.getByRole('heading', { name: '设计版本' })).toBeInTheDocument()
    expect(screen.getByText('下一设计版本')).toBeInTheDocument()
    expect(screen.getByText('v4')).toBeInTheDocument()

    await user.type(screen.getByLabelText('设计版本说明'), '调整戒臂弧度并重做面宽比例')
    await user.type(screen.getByLabelText('设计变更点'), '戒臂由直线改为微弧\n面宽比例重新调整')
    await user.type(screen.getByLabelText('设计版本相关文件'), 'ring-v4-design.pdf')
    await user.click(screen.getByRole('button', { name: '创建设计新版本' }))

    expect(screen.getByRole('heading', { name: '款式详情' })).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: '查看版本记录' }))

    expect(screen.getByText('v4 · 管理员')).toBeInTheDocument()
    expect(screen.getAllByText('调整戒臂弧度并重做面宽比例').length).toBeGreaterThan(0)
    expect(screen.getAllByText('ring-v4-design.pdf').length).toBeGreaterThan(0)
  })
  ...
  it('renders order-line center as one row per product', async () => {
    const user = userEvent.setup()
    renderRoute('/order-lines')

    expect(screen.getByRole('heading', { name: '销售中心' })).toBeInTheDocument()
    expect(screen.getByRole('columnheader', { name: '款式' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '全部销售' })).toBeInTheDocument()
    ...
    expect(screen.getAllByText('山形素圈戒指').length).toBeGreaterThan(0)
    expect(screen.getAllByText('如意吊坠').length).toBeGreaterThan(0)
    expect(screen.getByText('山形定制吊牌项链')).toBeInTheDocument()
    expect(screen.getByText('山形开口手链')).toBeInTheDocument()
    ...
    expect(within(ringRow).queryByText('内部 RING-SH-016')).not.toBeInTheDocument()
    await user.click(within(ringRow).getByRole('button', { name: '查看山形素圈戒指缩略图' }))
    expect(screen.getByRole('heading', { name: '山形素圈戒指 · RING-SH-016' })).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: '关闭' }))
    expect(within(ringRow).getByText('v3')).toBeInTheDocument()
    expect(within(ringRow).getByText('山形素圈戒指')).toBeInTheDocument()
  ...
  it('filters order-line center by category, owner, after-sales, urgent flag and keyword', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '全部销售' }))
    ...
    expect(screen.queryByText('如意吊坠')).not.toBeInTheDocument()
    ...
    await user.clear(screen.getByLabelText('负责人筛选'))
    await user.type(screen.getByLabelText('搜索货号 / 款式名称 / 客户 / 购买记录 / 平台单号'), 'TB-9938201')
    expect(screen.getAllByText('山形素圈戒指').length).toBeGreaterThan(0)
    expect(screen.getAllByText('如意吊坠').length).toBeGreaterThan(0)
    expect(screen.getByText('山形定制吊牌项链')).toBeInTheDocument()
  ...
  it('filters order-line center by factory, purchase and customer without legacy orders', async () => {
    ...
    expect(screen.getByText('山形素圈戒指')).toBeInTheDocument()
    expect(screen.queryByText('如意吊坠')).not.toBeInTheDocument()
    expect(screen.queryByText('山形定制吊牌项链')).not.toBeInTheDocument()
    ...
    expect(screen.getByText('暂无匹配销售')).toBeInTheDocument()
    expect(screen.getByText('当前筛选条件下没有销售，请放宽筛选或切回全部销售。')).toBeInTheDocument()
    expect(screen.queryByText('山形素圈戒指')).not.toBeInTheDocument()
  })
  ...
  it('opens order-line detail drawer from view button and switches records from row click', async () => {
    ...
    const topSummary = screen.getByText('顶部摘要').closest('section') as HTMLElement
    expect(topSummary).not.toBeNull()
    expect(within(topSummary).queryByText('销售编号')).not.toBeInTheDocument()
    expect(within(topSummary).queryByText('销售状态')).not.toBeInTheDocument()
    expect(within(topSummary).getByText('客服负责人')).toBeInTheDocument()
    expect(within(topSummary).getByText('王客服')).toBeInTheDocument()
    expect(within(topSummary).getByText('客户付款时间')).toBeInTheDocument()
    expect(within(topSummary).getByText('2026-04-21 10:45')).toBeInTheDocument()
    expect(within(topSummary).getByText('登记时间')).toBeInTheDocument()
    expect(within(topSummary).getByText('2026-04-21 10:46')).toBeInTheDocument()
    const priceFinance = screen.getByText('价格与财务').closest('section') as HTMLElement
    expect(priceFinance).not.toBeNull()
    expect(within(priceFinance).getByText('标价')).toBeInTheDocument()
    expect(within(priceFinance).getByText('定金')).toBeInTheDocument()
    expect(within(priceFinance).getByText('成交价')).toBeInTheDocument()
    expect(within(priceFinance).getByText('尾款')).toBeInTheDocument()
    expect(within(priceFinance).getByText('财务核算状态')).toBeInTheDocument()
    expect(screen.getAllByText('山形素圈戒指').length).toBeGreaterThan(0)
    const completenessDetails = screen.getByLabelText('资料完整度明细')
    expect(within(completenessDetails).getByText('款式名称')).toBeInTheDocument()
    expect(within(completenessDetails).getByText('材质')).toBeInTheDocument()
    expect(within(completenessDetails).getByText('刻字参考图')).toBeInTheDocument()
    ...
    expect(within(customParams).queryByText('款式别名')).not.toBeInTheDocument()
    expect(within(customParams).queryByText('生产备注')).not.toBeInTheDocument()
    expect(within(customParams).queryByText('承诺交期')).not.toBeInTheDocument()
    expect(within(customParams).getByText('来源款式编号')).toBeInTheDocument()
    expect(within(customParams).getByText('PD-RING-001')).toBeInTheDocument()
    expect(within(customParams).getByText('是否需要设计')).toBeInTheDocument()
    expect(within(customParams).getByText('是否需要建模')).toBeInTheDocument()
    expect(within(customParams).getByText('是否需要出蜡')).toBeInTheDocument()
    expect(screen.getByText('设计建模')).toBeInTheDocument()
    const designModeling = screen.getByText('设计建模').closest('section') as HTMLElement
    expect(designModeling).not.toBeNull()
    expect(within(designModeling).queryByText('是否需要设计')).not.toBeInTheDocument()
    expect(within(designModeling).queryByText('是否需要建模')).not.toBeInTheDocument()
    expect(within(designModeling).queryByText('是否需要出蜡')).not.toBeInTheDocument()
    expect(screen.queryByText('来源款式')).not.toBeInTheDocument()
    expect(screen.queryByText('规格参数')).not.toBeInTheDocument()
    expect(screen.queryByText('购买记录入口')).not.toBeInTheDocument()
    const purchaseEntry = screen.getByLabelText('购买记录入口')
    expect(within(purchaseEntry).getByText('PUR-202604-001')).toBeInTheDocument()
    ...
    await user.click(screen.getByRole('button', { name: '查看来源款式：山形素圈戒指' }))

    expect(screen.getByText('来源款式详情')).toBeInTheDocument()
    expect(screen.getByText('这里展示款式模板原始信息；查看或调整销售不会修改款式模板。')).toBeInTheDocument()
    expect(screen.getByText('销售参数对比')).toBeInTheDocument()
    ...
    expect(screen.getAllByText('如意吊坠').length).toBeGreaterThan(0)
    expect(screen.getAllByText('物流已创建').length).toBeGreaterThan(0)
    expect(screen.getByText('顺丰速运')).toBeInTheDocument()
    expect(screen.getByText('暂无售后记录')).toBeInTheDocument()
  })

  it('renders order-line detail route and missing detail empty state', () => {
    renderRoute('/order-lines/oi-ring-001')

    expect(screen.getAllByRole('heading', { name: '销售详情' }).length).toBeGreaterThan(0)
    expect(screen.getByText('RING-SH-016 · 山形素圈戒指')).toBeInTheDocument()
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
    expect(screen.getByText('顶部摘要')).toBeInTheDocument()
    expect(screen.getByText('客户与购买入口')).toBeInTheDocument()
    expect(screen.getByText('财务摘要')).toBeInTheDocument()
    expect(screen.getByText('库存追溯')).toBeInTheDocument()
    expect(screen.getByText('物流 / 售后')).toBeInTheDocument()
    expect(screen.getAllByRole('link', { name: '打开购买记录' })[0]).toHaveAttribute('href', '/purchases/o-202604-001')
    expect(screen.getByRole('link', { name: '返回销售中心' })).toHaveAttribute('href', '/order-lines')

    cleanup()

    renderRoute('/order-lines/not-found')

    expect(screen.getAllByRole('heading', { name: '销售详情' }).length).toBeGreaterThan(0)
    expect(screen.getByText('未找到销售')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: '返回销售中心' })).toHaveAttribute('href', '/order-lines')
  })
  ...
  it('edits basic order-line requirements from the detail drawer without changing siblings', async () => {
    ...
    await user.click(within(ringRow as HTMLElement).getByRole('button', { name: '查看' }))
    await user.click(screen.getByRole('button', { name: '编辑定制参数' }))
    ...
    await user.clear(screen.getByLabelText('特殊需求备注'))
    await user.type(screen.getByLabelText('特殊需求备注'), '客户改为 18K 微镶')
    ...
    expect(screen.getByRole('status')).toHaveTextContent('已保存定制参数')
    expect(screen.getByText('编辑销售需求')).toBeInTheDocument()
    expect(screen.getByText('修改了销售定制参数')).toBeInTheDocument()
    ...
    expect(within(ringRow as HTMLElement).getByText((content) => content.includes('18K金') && content.includes('微镶') && !content.includes('材质'))).toBeInTheDocument()
    expect(within(pendantRow as HTMLElement).getByText('如意吊坠')).toBeInTheDocument()
    expect(within(pendantRow as HTMLElement).queryByText('赵客服')).not.toBeInTheDocument()
  })

  it('edits design and modeling info from the detail drawer', async () => {
    const user = userEvent.setup()
    renderRoute('/order-lines')
    ...
    expect(screen.queryByLabelText('是否需要设计')).not.toBeInTheDocument()
    expect(screen.queryByLabelText('是否需要建模')).not.toBeInTheDocument()
    expect(screen.queryByLabelText('是否需要出蜡')).not.toBeInTheDocument()

    await user.selectOptions(screen.getByLabelText('设计状态'), 'revision_requested')
    await user.clear(screen.getByLabelText('设计负责人'))
    await user.type(screen.getByLabelText('设计负责人'), '周设计')
    expect(screen.queryByLabelText('建模文件链接')).not.toBeInTheDocument()
    expect(screen.queryByLabelText('出蜡文件链接')).not.toBeInTheDocument()
    await user.upload(screen.getByLabelText('建模文件上传'), new File(['model'], 'ring-model-upload-v4.3dm', { type: 'application/octet-stream' }))
    await user.upload(screen.getByLabelText('出蜡文件上传'), new File(['wax'], 'ring-wax-output-v4.stl', { type: 'application/octet-stream' }))
    await user.click(screen.getByRole('button', { name: '保存设计建模' }))

    expect(screen.getByRole('status')).toHaveTextContent('已保存设计建模信息')
    expect(screen.getByText('需修改')).toBeInTheDocument()
    expect(screen.getByText('周设计')).toBeInTheDocument()
    expect(screen.getByText('ring-model-upload-v4.3dm')).toBeInTheDocument()
    expect(screen.getByText('ring-wax-output-v4.stl')).toBeInTheDocument()
  })
  ...
  it('edits outsource info from the detail drawer without changing siblings', async () => {
    ...
    await user.clear(screen.getByLabelText('货号'))
    await user.type(screen.getByLabelText('货号'), 'SKU-RING-OUT-01')
    ...
    expect(screen.getByText('修改了销售跟单 / 下厂信息')).toBeInTheDocument()
  ...
  it('edits factory feedback from the detail drawer without changing siblings', async () => {
    ...
    expect(screen.getByText('修改了销售工厂回传信息')).toBeInTheDocument()
  ...
  it('adds logistics to only the selected order line from the detail drawer', async () => {
    ...
    expect(within(ringRow as HTMLElement).getByText('物流已创建')).toBeInTheDocument()
    expect(within(pendantRow as HTMLElement).getByText('物流已创建')).toBeInTheDocument()
  ...
  it('shows non-referenced custom order line details in drawer', async () => {
    ...
    expect(screen.getByRole('dialog')).toBeInTheDocument()
    expect(screen.getAllByText('山形定制吊牌项链').length).toBeGreaterThan(0)
    const customParams = screen.getByText('定制参数').closest('section')
    expect(customParams).not.toBeNull()
    expect(within(customParams as HTMLElement).getByText('来源款式编号')).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '查看来源款式：山形定制吊牌项链' })).not.toBeInTheDocument()
  ...
  it('renders purchase detail as the current grouping route', () => {
    ...
    expect(screen.getByText('本页只做购买记录归组；每条销售独立推进执行。')).toBeInTheDocument()
    expect(screen.getByText('销售数量')).toBeInTheDocument()
    expect(screen.getByText('本次销售列表')).toBeInTheDocument()
    expect(screen.getByText('实付款 / 成交价')).toBeInTheDocument()
    expect(screen.getAllByText('山形素圈戒指').length).toBeGreaterThan(0)
    expect(screen.getAllByText('如意吊坠').length).toBeGreaterThan(0)
    expect(screen.getByText('山形定制吊牌项链')).toBeInTheDocument()
    expect(screen.getByText('生产中')).toBeInTheDocument()
    expect(screen.getAllByText('待财务确认').length).toBeGreaterThan(0)
  ...
  it('renders current workflow entry routes without using legacy orders as the main entry', () => {
    const entries = [
      ...
      { path: '/production-plan/task-factory-001', heading: '货号详情' }
    ]
  ...
  it('opens order-line drawer from purchase detail line table', async () => {
    ...
    await user.click(within(pendantRow as HTMLElement).getByRole('button', { name: '查看销售' }))
  ...
  it('updates one order-line status from purchase detail without changing siblings', async () => {
    ...
    await user.click(within(necklaceRow as HTMLElement).getByRole('button', { name: '查看销售' }))
  ...
  it('edits order-line requirements from purchase detail and keeps siblings unchanged', async () => {
    ...
    await user.click(within(necklaceRow as HTMLElement).getByRole('button', { name: '查看销售' }))
    await user.click(screen.getByRole('button', { name: '编辑定制参数' }))
    await user.clear(screen.getByLabelText('款式名称'))
    await user.type(screen.getByLabelText('款式名称'), '定制项链改版')
    await user.clear(screen.getByLabelText('材质'))
    ...
    expect(within(necklaceRow as HTMLElement).getByText((content) => content.includes('铂金') && !content.includes('材质'))).toBeInTheDocument()
    expect(within(ringRow as HTMLElement).getAllByText('山形素圈戒指').length).toBeGreaterThan(0)
    expect(within(pendantRow as HTMLElement).getAllByText('如意吊坠').length).toBeGreaterThan(0)
  ...
  it('edits outsource info from purchase detail and keeps siblings unchanged', async () => {
    ...
    await user.clear(screen.getByLabelText('货号'))
    await user.type(screen.getByLabelText('货号'), 'SKU-NECK-OUT-01')
    await user.clear(screen.getByLabelText('工厂计划交期'))
  ...
  it('edits factory feedback from purchase detail and keeps siblings unchanged', async () => {
    ...
    expect(within(necklaceRow as HTMLElement).queryByText('工厂 工厂生产中')).not.toBeInTheDocument()
    expect(within(ringRow as HTMLElement).getAllByText('山形素圈戒指').length).toBeGreaterThan(0)
  ...
  it('creates a purchase draft with multiple order-line cards', async () => {
    ...
    expect(screen.getByText('先填写本次购买的公共信息，再逐件添加销售。')).toBeInTheDocument()
    expect(screen.getByText('购买公共信息')).toBeInTheDocument()
    const generatedPurchaseNoInput = screen.getByLabelText('购买记录编号') as HTMLInputElement
    expect(generatedPurchaseNoInput.value).toMatch(/^PUR-\d{6}-002$/)
    expect(screen.getByText('客户与收货信息')).toBeInTheDocument()
    expect(screen.getByText('付款信息')).toBeInTheDocument()
    expect(screen.getByText('销售区域')).toBeInTheDocument()
    expect(screen.getByText('本次购买共 1 条销售')).toBeInTheDocument()
    expect(screen.getByText('销售草稿 1')).toBeInTheDocument()
    ...
    await user.click(screen.getByRole('button', { name: '添加销售' }))
    await user.click(screen.getByRole('button', { name: '添加销售' }))

    expect(screen.getByText('本次购买共 3 条销售')).toBeInTheDocument()
    expect(screen.getByText('销售草稿 1')).toBeInTheDocument()
    expect(screen.getByText('销售草稿 2')).toBeInTheDocument()
    expect(screen.getByText('销售草稿 3')).toBeInTheDocument()
    expect(screen.getByText('当前草稿：1 笔购买记录 + 3 条销售。')).toBeInTheDocument()
    expect(screen.getByText('¥ 6,000')).toBeInTheDocument()

    const firstLineCard = screen.getByText('销售草稿 1').closest('.subtle-panel')
    const secondLineCard = screen.getByText('销售草稿 2').closest('.subtle-panel')
    const thirdLineCard = screen.getByText('销售草稿 3').closest('.subtle-panel')
    expect(firstLineCard).not.toBeNull()
    expect(secondLineCard).not.toBeNull()
    expect(thirdLineCard).not.toBeNull()

    await user.type(within(firstLineCard as HTMLElement).getByLabelText('引用款式'), '山形')
    await user.click(within(firstLineCard as HTMLElement).getByRole('button', { name: '预览山形素圈戒指' }))
    expect(within(firstLineCard as HTMLElement).getByLabelText('来源款式预览')).toBeInTheDocument()
    await user.click(within(firstLineCard as HTMLElement).getByRole('button', { name: '选择山形素圈戒指' }))
    expect(within(firstLineCard as HTMLElement).getByLabelText('款式名称')).toHaveValue('山形素圈戒指')
    expect(within(firstLineCard as HTMLElement).getByText(/来源款式：山形素圈戒指/)).toBeInTheDocument()
    ...
    expect(within(firstLineCard as HTMLElement).getByText(/资料完整度 6\/8/)).toBeInTheDocument()
    expect(within(firstLineCard as HTMLElement).getByText(/缺失：刻字参考图、刻字 PLT 文件/)).toBeInTheDocument()
    await user.upload(within(firstLineCard as HTMLElement).getByLabelText('刻字参考图'), new File(['image'], 'ring-engrave-reference.jpg', { type: 'image/jpeg' }))
    await user.upload(within(firstLineCard as HTMLElement).getByLabelText('刻字 PLT 文件'), new File(['plt'], 'ring-engrave.plt', { type: 'application/octet-stream' }))
    expect(within(firstLineCard as HTMLElement).getByText('ring-engrave-reference.jpg')).toBeInTheDocument()
    expect(within(firstLineCard as HTMLElement).getByText('ring-engrave.plt')).toBeInTheDocument()
    expect(within(firstLineCard as HTMLElement).getByText(/资料完整度 8\/8/)).toBeInTheDocument()
    ...
    await user.click(within(firstLineCard as HTMLElement).getByRole('button', { name: '查看来源款式' }))
    expect(screen.getByText('来源款式详情')).toBeInTheDocument()
    expect(screen.getByText('PD-RING-001')).toBeInTheDocument()
    const firstLineGoodsNo = (within(firstLineCard as HTMLElement).getByLabelText('货号') as HTMLInputElement).value
    expect(firstLineGoodsNo).toMatch(/^SKU-202604-\d{3}$/)
    expect(firstLineGoodsNo).not.toBe('PD-RING-001')
    expect(within(firstLineCard as HTMLElement).queryByText('产品货号')).not.toBeInTheDocument()
    expect(within(firstLineCard as HTMLElement).getByLabelText('款式名称')).toHaveValue('山形素圈戒指')
    ...
    await user.type(within(secondLineCard as HTMLElement).getByLabelText('引用款式'), '如意')
    await user.click(within(secondLineCard as HTMLElement).getByRole('button', { name: '预览如意吊坠' }))
    expect(within(secondLineCard as HTMLElement).getByLabelText('来源款式预览')).toBeInTheDocument()
    await user.click(within(secondLineCard as HTMLElement).getByRole('button', { name: '选择如意吊坠' }))
    expect(within(secondLineCard as HTMLElement).getByLabelText('款式名称')).toHaveValue('如意吊坠')
    expect(within(secondLineCard as HTMLElement).getByText(/来源款式：如意吊坠/)).toBeInTheDocument()
    ...
    await user.click(within(secondLineCard as HTMLElement).getByRole('button', { name: '查看来源款式' }))
    expect(screen.getByText('PD-PENDANT-001')).toBeInTheDocument()
    expect(screen.getAllByText('小号').length).toBeGreaterThan(0)
    await user.click(screen.getAllByRole('button', { name: '关闭' })[0] as HTMLElement)

    await user.type(within(thirdLineCard as HTMLElement).getByLabelText('款式名称'), '定制项链')

    await user.click(screen.getByRole('button', { name: '保存草稿' }))

    expect(await screen.findByRole('heading', { name: '购买记录详情' })).toBeInTheDocument()
    expect(logSpy).not.toHaveBeenCalled()
    expect(screen.getByText(generatedPurchaseNoInput.value)).toBeInTheDocument()
    expect(screen.getByText('TB-202604-NEW')).toBeInTheDocument()
    expect(screen.getByText(firstLineGoodsNo)).toBeInTheDocument()
    expect(screen.getAllByText('如意吊坠').length).toBeGreaterThan(0)
    expect(screen.getAllByText('定制项链').length).toBeGreaterThan(0)

    await user.click(screen.getByRole('link', { name: '销售' }))
    expect(screen.getByText(firstLineGoodsNo)).toBeInTheDocument()
    expect(screen.getAllByText('定制项链').length).toBeGreaterThan(0)

    await user.click(screen.getByRole('link', { name: '工作' }))
    expect(screen.getByText(generatedPurchaseNoInput.value)).toBeInTheDocument()
    expect(screen.getByText('销售：3 条')).toBeInTheDocument()
  })
  ...
  it('validates, duplicates and removes purchase draft order-line cards', async () => {
    ...
    const firstLineCard = screen.getByText('销售草稿 1').closest('.subtle-panel')
    expect(firstLineCard).not.toBeNull()
    expect(within(firstLineCard as HTMLElement).getByRole('button', { name: '删除销售' })).toBeDisabled()

    await user.type(within(firstLineCard as HTMLElement).getByLabelText('引用款式'), '山形')
    await user.click(within(firstLineCard as HTMLElement).getByRole('button', { name: '选择山形素圈戒指' }))
    expect(screen.queryByRole('alert')).not.toBeInTheDocument()

    await user.click(within(firstLineCard as HTMLElement).getByRole('button', { name: '改为手动填写' }))
    await user.type(within(firstLineCard as HTMLElement).getByLabelText('款式名称'), '手动定制戒指')
    await user.click(within(firstLineCard as HTMLElement).getByRole('button', { name: '复制销售' }))

    expect(screen.getByText('本次购买共 2 条销售')).toBeInTheDocument()
    const secondLineCard = screen.getByText('销售草稿 2').closest('.subtle-panel')
    expect(secondLineCard).not.toBeNull()
    expect(within(secondLineCard as HTMLElement).getByLabelText('款式名称')).toHaveValue('手动定制戒指')

    await user.click(within(secondLineCard as HTMLElement).getByRole('button', { name: '删除销售' }))
    expect(screen.getByText('本次购买共 1 条销售')).toBeInTheDocument()
    expect(screen.queryByText('销售草稿 2')).not.toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: '保存草稿' }))
    expect(await screen.findByRole('heading', { name: '购买记录详情' })).toBeInTheDocument()
    expect(screen.getAllByText('手动定制戒指').length).toBeGreaterThan(0)
    expect(logSpy).not.toHaveBeenCalled()
  })
  ...
  it('renders lightweight customer center list from current mainline data', () => {
    ...
    expect(screen.getByText('当前销售')).toBeInTheDocument()
    ...
    expect(screen.getAllByRole('link', { name: '查看客户' })[0]).toHaveAttribute('href', '/customers/customer-zhang-001')
  })
  ...
  it('renders customer detail with purchase, order-line and after-sales history', () => {
    ...
    expect(screen.getByText('历史销售')).toBeInTheDocument()
    expect(screen.getByText('RING-SH-016')).toBeInTheDocument()
    expect(screen.getByText('PDT-SH-S')).toBeInTheDocument()
    expect(screen.getByText('NECK-CUSTOM-042')).toBeInTheDocument()
    expect(screen.getByText('WAX-BR-001')).toBeInTheDocument()
    expect(screen.getAllByRole('link', { name: '查看销售' })[0]).toHaveAttribute('href', '/order-lines/oi-ring-001')
  ...
  it('shows current customer aggregation counts', () => {
  ...
  it('renders task list route', () => {
    ...
    expect(screen.getByText('销售任务分组')).toBeInTheDocument()
    expect(screen.getByText('待建模')).toBeInTheDocument()
    expect(screen.getByText('待财务确认')).toBeInTheDocument()
    expect(screen.getByText('确认戒指最终圈号')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: '返回销售中心' })).toHaveAttribute('href', '/order-lines')
    expect(screen.getAllByRole('link', { name: 'RING-SH-016 · 山形素圈戒指' })[0]).toHaveAttribute('href', '/order-lines/oi-ring-001')
  ...
  it('renders task detail route', () => {
    ...
    expect(screen.getAllByText('PUR-202604-001').length).toBeGreaterThan(0)
    expect(screen.getAllByRole('button', { name: '查看销售' }).length).toBeGreaterThan(0)
    expect(screen.getAllByRole('link', { name: 'PUR-202604-001' })[0]).toHaveAttribute('href', '/purchases/o-202604-001')
    expect(screen.queryByText('订单商品')).not.toBeInTheDocument()
    expect(screen.queryByText('返回订单')).not.toBeInTheDocument()
    expect(screen.queryByText('查看订单')).not.toBeInTheDocument()
  ...
  it('renders production follow-up tabs from current order lines', async () => {
    ...
    expect(screen.getByText('复古小花耳钉')).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: '切换到待下发生产' }))
    expect(screen.queryByText('山形胸针')).not.toBeInTheDocument()
  ...
  it('updates production follow-up status through current order-line state', async () => {
    ...
    const reviewCard = screen.getByText('复古小花耳钉').closest('.workbench-task-card')
    ...
    const earringRow = screen.getByText('复古小花耳钉').closest('.workbench-task-card')
  ...
  it('blocks and resumes production follow-up through action availability', async () => {
    ...
    expect(within(producingCard as HTMLElement).queryByRole('button', { name: '恢复生产' })).not.toBeInTheDocument()
    ...
    expect(within(blockedCard as HTMLElement).queryByRole('button', { name: '标记阻塞' })).not.toBeInTheDocument()
  ...
  it('renders design modeling workbench from current order lines', async () => {
    ...
    const revisionCard = screen.getByText('山形定制吊牌项链').closest('.workbench-task-card')
  ...
  it('updates design and modeling status from the workbench', async () => {
    ...
    expect(screen.getByText('山形定制吊牌项链')).toBeInTheDocument()
    const updatedNecklaceRow = screen.getByText('山形定制吊牌项链').closest('.workbench-task-card')
  ...
  it('renders factory center without customer or finance fields', async () => {
    ...
    expect(screen.getByText('山形胸针试产版')).toBeInTheDocument()
    ...
    expect(screen.queryByText('8500')).not.toBeInTheDocument()
    expect(screen.queryByText('财务备注')).not.toBeInTheDocument()
  ...
  it('updates factory production return through current order-line state', async () => {
    ...
    const producingRow = screen.getByText('山形胸针试产版').closest('.workbench-task-card')
    expect(producingRow).not.toBeNull()
    ...
    const returnRow = screen.getByText('山形胸针试产版').closest('.workbench-task-card')
    ...
    expect(within(returnRow as HTMLElement).getByRole('alert')).toHaveTextContent('总重必填')
    expect(within(returnRow as HTMLElement).getByRole('alert')).toHaveTextContent('净金重必填')
    expect(within(returnRow as HTMLElement).getByRole('alert')).toHaveTextContent('基础工费必填')
    expect(within(returnRow as HTMLElement).getByRole('alert')).toHaveTextContent('成品图文件必填')
    expect(within(returnRow as HTMLElement).getByRole('alert')).toHaveTextContent('结算单文件必填')
    ...
    expect(screen.getByText('山形胸针试产版')).toBeInTheDocument()
    expect(screen.getByText('成品图：pin-finished.jpg')).toBeInTheDocument()
    expect(screen.getByText('结算单：pin-settlement.pdf')).toBeInTheDocument()
    expect(container.querySelector('a[href^="/orders"]')).toBeNull()
  })
  ...
  it('blocks finance settlement confirmation until payments and risks are cleared', async () => {
    const user = userEvent.setup()
    renderRoute('/finance')

    await user.click(screen.getByRole('button', { name: '切换到补款 / 退款' }))
    const reviewRow = screen.getByText('山形素圈戒指').closest('.workbench-task-card')
    expect(reviewRow).not.toBeNull()
    await user.click(within(reviewRow as HTMLElement).getByRole('button', { name: /展开/ }))
    await user.click(within(reviewRow as HTMLElement).getByRole('button', { name: '确认工厂结算' }))

    expect(within(reviewRow as HTMLElement).getByRole('alert')).toHaveTextContent('尾款未收齐')
    expect(within(reviewRow as HTMLElement).getByRole('alert')).toHaveTextContent('补 / 退款未复核')
    expect(within(reviewRow as HTMLElement).getByRole('alert')).toHaveTextContent('收退款异常未解除')
    expect(screen.getByRole('heading', { name: '补款 / 退款' })).toBeInTheDocument()
    expect(within(reviewRow as HTMLElement).getByRole('button', { name: '确认工厂结算' })).not.toBeDisabled()
  })

  it('can resolve a manually marked finance abnormal line back to pending settlement', async () => {
    const user = userEvent.setup()
    renderRoute('/finance')
    ...
    await user.type(within(pendantRow as HTMLElement).getByLabelText('异常原因-oi-pendant-001'), '结算金额待复核')
    await user.click(within(pendantRow as HTMLElement).getByRole('button', { name: '标记财务异常' }))

    const abnormalRow = screen.getByText('如意吊坠').closest('.workbench-task-card')
    expect(abnormalRow).not.toBeNull()
    expect(screen.getByText('结算金额待复核')).toBeInTheDocument()
    await user.click(within(abnormalRow as HTMLElement).getByRole('button', { name: '解除财务异常' }))

    expect(screen.getByRole('heading', { name: '待工厂结算确认' })).toBeInTheDocument()
    expect(screen.getByText('如意吊坠')).toBeInTheDocument()
    expect(screen.queryByText('结算金额待复核')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '解除财务异常' })).not.toBeInTheDocument()
  })
  ...
  it('confirms finance settlement without changing production ownership', async () => {
    ...
    expect(screen.getByText('如意吊坠')).toBeInTheDocument()
    expect(screen.getByText('毛利 ¥280')).toBeInTheDocument()
    expect(screen.getByText('毛利率 21.9%')).toBeInTheDocument()
  ...
  it('renders management dashboard from current workflow metrics', () => {
    ...
    expect(screen.getByText('销售状态分布')).toBeInTheDocument()
  ...
  it('switches mock roles and limits primary navigation by capability', async () => {
    ...
    expect(nav.getByRole('link', { name: /销售/ })).toBeInTheDocument()
    expect(nav.queryByRole('link', { name: /财务/ })).not.toBeInTheDocument()
  ...
  it('records inventory inbound and movement actions locally', async () => {
    const user = userEvent.setup()
    renderRouteWithTestNavigation('/inventory')

    await user.type(screen.getByLabelText('款式名称'), '设计部门未售样戒')
    ...
    await user.selectOptions(screen.getByLabelText('关联销售'), 'oi-ring-001')
    ...
    await user.click(screen.getByRole('button', { name: '测试跳转财务' }))
    await user.click(screen.getByRole('button', { name: '切换到待收款' }))
    await user.click(screen.getAllByRole('button', { name: /RING-SH-016/ })[0])
    expect(screen.getByText('FIFO库存领用成本：¥150')).toBeInTheDocument()
  })
  ...
  it('renders production plan list route without requiring factory role', () => {
    ...
    expect(screen.getAllByRole('link', { name: '山形素圈戒指' })[0]).toHaveAttribute('href', '/order-lines/oi-ring-001')
    expect(screen.getByRole('link', { name: '查看销售' })).toHaveAttribute('href', '/order-lines/oi-ring-001')
  ...
  it('renders production plan detail route', () => {
    ...
    expect(screen.getByRole('heading', { name: '货号详情' })).toBeInTheDocument()
    expect(screen.getByText('来源追溯')).toBeInTheDocument()
    expect(screen.getAllByText('PD-RING-001').length).toBeGreaterThan(0)
    expect(screen.getByText('购买记录')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'PUR-202604-001' })).toHaveAttribute('href', '/purchases/o-202604-001')
    expect(screen.getAllByText('销售').length).toBeGreaterThan(0)
    expect(screen.getByRole('button', { name: '查看销售' })).toBeInTheDocument()
  ...
  it('updates production plan status actions through order-line production info', async () => {
    ...
    await user.clear(screen.getByLabelText('工厂备注'))
    await user.type(screen.getByLabelText('工厂备注'), '生产计划详情页写回销售生产信息')

    expect(screen.getByLabelText('回传重量')).toHaveValue('5.8g')
    expect(screen.getByLabelText('质检结论')).toHaveValue('生产计划回传通过')
    expect(screen.getByLabelText('工厂备注')).toHaveValue('生产计划详情页写回销售生产信息')
    expect(container.querySelector('a[href^="/orders"]')).toBeNull()
  })
