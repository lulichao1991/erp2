...
`InventoryItem` 可以引用 `Product / Purchase / OrderLine / Customer`，但它不是产品模板，也不是商品行执行对象，不驱动商品行状态流转。
...
- `/inventory` 基于 `InventoryItem` 做库管库存资产台账，记录设计留样、客户退货和其他库存；支持前端 mock 入库、占用、释放、出库、报废、库位调整、质检处置和盘点，但不推进商品行执行状态
...
- 工厂协同中心只允许展示分配给工厂的生产任务、设计 / 建模 / 出蜡资料和生产回传字段，不得展示客户联系方式、地址、销售价格、定金、尾款、利润或财务备注
- 财务中心允许查看金额、成本和工厂回传数据，但不得负责推进设计、建模或生产执行字段
- 商品行资料完整性、生产逾期、工厂回传异常、财务异常和角色待办徽标应优先复用 `orderLineRiskSelectors`
