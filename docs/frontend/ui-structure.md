...
## 销售详情 `/order-lines/:orderLineId`

单件销售详情页仍以 `OrderLine` 为主语，复用销售详情抽屉能力承接状态推进、物流、售后、设计建模、跟单和工厂回传。找不到 `orderLineId` 时展示空状态并返回销售中心，不静默跳转到旧入口。
...
- 可以添加多条销售草稿。
- 引用款式入口支持按名称 / 编号 / 系列搜索、按品类筛选，并在销售草稿卡内预览来源款式的版本、规格、材质、工艺和基础价区间。
...
- 平台店铺商品当前是产品扩展区 mock 展示，不进入正式 `Product` 类型，也不参与报价、销售执行或财务核算。
...
| 页面 | 只处理 |
|---|---|
| `/production-follow-up` | 跟单审核、生产下发、完工审核、阻塞处理 |
| `/design-modeling` | 设计/建模状态和文件 |
| `/factory` | 工厂接收、生产、回传、异常标记、恢复生产和回传附件 |
| `/finance` | 按货号处理应收、收款、退款、净收、待收、补款 / 退款复核、退款原因风险、成本卡、旧金抵扣追溯、FIFO 库存领用成本、结算、财务异常和锁定后只读 |
| `/inventory` | 库存资产、旧金入库追溯、FIFO 批次和库存流转 |
| `/management` | 汇总观察 |
