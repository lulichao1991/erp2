...
核心 workflow 状态字段 `lineStatus / designStatus / modelingStatus / productionStatus / factoryStatus / financeStatus` 已冻结为受控 union，不再接受任意 `string`。真实接口返回未知状态时，必须在接口边界映射、拒绝或先更新字段契约，不能让未知状态直接进入 `OrderLine` current model。

工厂回传边界固定为：`productionInfo` 只保存展示型回传文本和 `feedbackStatus`；`productionData` 保存结构化重量、成本、附件和结算数据。工厂提交回传只允许进入 `factory_returned` 并写生产回传资料，不允许写 `financeStatus`。跟单审核回传时由 `approveFactoryReturn` 根据销售金额、报价、已收/待收金额、工厂结算或生产成本判断是否进入财务确认；只有明确 `financeStatus = not_required` 且无金额/无成本数据的销售可以跳过财务。财务边界固定为：`FinancePaymentRecord` 是商品行收付款流水权威来源；`Purchase.finance` 只做整笔购买聚合摘要，不驱动商品行财务计算。

## 隐式模型映射

当前前端不额外引入复杂后端模型名，真实接口评审时按下列映射理解：

| 后续候选模型 | 当前承载位置 | 说明 |
|---|---|---|
| `ProductVariant` | `ProductSpecRow` | 当前规格行承载材质/尺寸/工艺版本选择、基础价格和规格参数；当前阶段不新增独立 variant 模块。 |
| `FactoryFeedback` | `OrderLine.productionInfo / OrderLine.productionData` | `productionInfo` 放展示型回传文本和 `feedbackStatus`；`productionData` 放结构化重量、工费、附件和出货数据。 |
| `FinanceRecord` | `FinancePaymentRecord + OrderLine.financeStatus / financeNote / financeLocked / financeAbnormalReason` | 收退款流水是 `FinancePaymentRecord`，销售维度确认、异常和锁定状态留在 `OrderLine`。 |

## 字段契约矩阵

| 阶段 | 必填主对象 | 必填字段 / 记录 | 不做 |
|---|---|---|---|
| create | `Purchase` + `OrderLine` drafts | `Purchase.purchaseNo / customerId / sourceChannel / purchaseType`；每条草稿输出 `orderLineDrafts` 并包含 `productionTaskNo / name / quantity / requiresDesign / requiresModeling / requiresWax` | 不创建 legacy `/orders`，不把单件执行字段塞回 `Purchase` |
| 客服确认 | `OrderLine` | 商品资料完整度、`requiresDesign / requiresModeling`、`lineStatus` 分流结果、必要的 `actualRequirements` | 不跳过设计/建模分流规则 |
| 设计 / 建模 | `OrderLine` | 顶层 `designStatus / modelingStatus`、设计/建模文件、修改原因、出蜡文件记录 | 不写 `productionInfo.factoryStatus`，不展示财务金额 |
| 跟单下发 | `OrderLine` | `productionStatus / factoryStatus / factoryId / productionSentAt / factoryPlannedDueDate`、生产备注 | 不回写旧订单模型 |
| 工厂回传 | `OrderLine.productionInfo / productionData` | `feedbackStatus / totalWeight / netMetalWeight / actualMaterial / baseLaborCost / extraLaborCost / totalLaborCost / shippedAt / finishedImageUrls / settlementFileUrls`；提交前校验总重、净金重、实际材质、基础工费、成品图和结算单 | 工厂不写 `financeStatus`，不展示客户联系方式和销售价格 |
| 财务确认 | `FinancePaymentRecord` + `OrderLine.finance*` | `orderLineId / amount / method / recordType / reviewStatus`、`financeStatus / financeConfirmedAt / financeLocked / financeNote / factorySettlementAmount`；确认前必须校验尾款已收齐、补 / 退款已复核、退款原因和财务风险已解除、工厂结算金额有效 | 财务不推进设计、建模、生产或工厂字段 |
| 发货完成 | `LogisticsRecord` + `OrderLine` | 物流 `orderLineId / logisticsType / direction / company / trackingNo / shippedAt / signedAt`；完成销售只能由 `completeOrderLine` 从 `ready_to_ship` 进入 `completed` | 物流签收、库存出库或售后关闭不得自动完成销售 |
...
物流、售后和库存是旁路记录：`LogisticsRecord / AfterSalesCase` 必须 `orderLineId` 优先关联；库存只有共享 `useAppData.inventoryMovements` 中的 `InventoryMovement.type = outbound` 且 `relatedOrderLineId` 指向当前销售时，FIFO 成本才进入财务成本卡。其他库存动作不得反向推动客服、设计、建模、生产、工厂或财务状态。
...
- `InventoryMovement` 可以登记 `reserve / release / adjust / scrap / outbound`；只有共享库存流水中的 `outbound + relatedOrderLineId` 进入财务成本卡，不推进主流程。
...
- 工厂回传动作不得写 `financeStatus`；财务确认入口只能由跟单审核回传后的规则或财务动作产生。
- 财务收款流水必须关联 `orderLineId`；`Purchase.finance` 只做聚合摘要。
- FIFO 成本只来自共享库存状态中关联销售的 `InventoryMovement.type = outbound`。
