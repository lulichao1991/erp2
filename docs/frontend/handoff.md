...
- 仓库商品管理第一版，以 `InventoryItem` 记录设计留样、客户退货、常备采购和其他库存，并支持总数 / 可用数 / 已占用数展示、库管快捷视图、可领用 / 待出库 / 待盘点视图、低库存预警、库存详情来源追溯、关联商品行占用 / 出库追溯、质检处置、库存盘点、可关联商品行的前端 mock 库存流转记录和流转筛选
...
- 左侧导航（工作台 / 商品行中心 / 产品管理）当前占宽偏大，需要缩窄
...
**把商品行中心和产品管理真正通过“产品引用 → 规格选择 → 自动带价 → 来源产品核对”这条链路连起来。**
...
- `/order-lines`：商品行中心，一行代表一件商品
- `/purchases/:purchaseId`：购买记录详情，作为一次购买的归组页
- `/purchases/new`：新建购买记录草稿，支持 1 笔购买记录下添加多条商品行

本轮已在 `/purchases/new` 的商品行草稿卡中接入产品引用第一版：

- 每条商品行草稿可独立引用现有产品 mock
...
- `/purchases/new` 商品行草稿卡已接入
- `/order-lines` 商品行详情抽屉已接入
- `/purchases/:purchaseId` 复用的商品行详情抽屉已接入
...
本轮新增 `SourceProductDrawer`，用于展示产品模板原始信息，并从商品行或商品行草稿带入当前实际参数做轻量对比。

已接入入口：

- `/purchases/new` 商品行草稿卡：引用产品后显示“查看来源产品”
- `/order-lines` 商品行详情抽屉：来源产品按钮可打开模板详情
- `/purchases/:purchaseId` 商品行详情抽屉：复用同一个商品行详情抽屉，因此同样支持查看来源产品

抽屉口径：

- `SourceProductDrawer` 展示的是 `Product` 模板，不是商品行执行详情
- 对比区只传入当前商品行 / 商品行草稿的实际选择值
- 不把产品模板字段写回商品行状态
...
下一步建议进入商品行状态推进第一版，但仍保持 mock 前端态，不要提前扩展复杂权限或审批流。

---

### 15.6 商品行状态推进第一版

本轮在新版商品行详情抽屉中新增轻量状态推进：

- `/order-lines` 打开商品行详情后，可选择目标状态并更新
- `/purchases/:purchaseId` 打开商品行详情后，复用同一套状态更新控件
- 更新只发生在当前页面的前端 state 中，不写回 mock 文件，不接真实后端

实现口径：

- 商品行列表和购买记录详情页都以 mock 作为初始值
...
### 15.11 商品行跟单 / 下厂信息编辑第一版

本轮将商品行详情抽屉里的“跟单 / 下厂”区块从只读展示，推进为可维护区块。

新增能力：

- 在商品行详情抽屉新增“跟单 / 下厂”编辑入口
- 区块支持编辑、保存、取消
- 保存后只更新当前商品行
- `/order-lines` 列表同步更新跟单负责人和工厂名称
- `/purchases/:purchaseId` 购买记录详情页中的当前商品行同步更新跟单负责人
- 保存后追加一条 `OrderLineLog`
...
### 15.13 商品行物流 / 售后记录维护第一版

本轮将商品行详情抽屉里的物流和售后从“新增 / 查看”推进为记录级维护能力。
